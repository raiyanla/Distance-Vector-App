...
Dracula.prototype = {
  ...
  addinEdge: function(source, target, style) {
    var s = this.addNode(source);
    var t = this.addNode(target);
    var edge = new Dracula.Edge({ source: s, target: t, style: style });
    //s.edges.pop();
  ...
  removeNode: function(id) {
    ...
    for(var i = 0; i < this.edges.length; i++) {
      if ((this.edges[i].source.id == id.id || this.edges[i].target.id == id.id)) {
        console.log("TESTING");
    ...
    //this.edges[5].connection && this.edges[5].connection.label.hide();
